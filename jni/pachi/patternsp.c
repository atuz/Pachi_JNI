...
spatial_dict_init(bool will_append, bool hash)
{
	...
	char fname[1024];
    snprintf(fname, 1024, "%s/%s",patternpath,spatial_dict_filename);
	FILE *f = fopen(fname, "r");
	if (!f) {
		if (DEBUGL(1))
			fprintf(stderr, "No spatial dictionary %s, will not match spatial pattern features.\n",fname);
		return NULL;
	}
